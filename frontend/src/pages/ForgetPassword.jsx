...
const ForgetPassword = () => {
    ...
    //step 1
     const sendotp=async()=>{
      setloading(true)
     try {
      const response=await axios.post(`${serverURL}/api/auth/sentotp`,{email},{
...
//step2
 const verifyotp=async()=>{
   setloading(true)
     try {
      const response=await axios.post(`${serverURL}/api/auth/otpverify`,{email,otp},{
        withCredentials:true
      })
      if(response.status===201 || response.status ===200){
        toast.success(response?.data?.message)
     ...
     try {
      const response=await axios.post(`${serverURL}/api/auth/resetpassword`,{email,password:newpassword},{
        withCredentials:true
      })
      if(response.status===201 || response.status ===200){
        toast.success(response.data.message)
